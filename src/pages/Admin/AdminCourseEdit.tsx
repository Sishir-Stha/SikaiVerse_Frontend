...
import { ArrowLeft, Plus, Edit2, Trash2, ChevronDown, ChevronUp, Save, Loader2, Users } from 'lucide-react'
...
const mapApiToLocalCourse = (apiData: EditInfo, instructors: Instructor[]): LocalCourse => {
  // Find instructor ID by name
  const instructor = instructors.find(i => i.fullName === apiData.instructorName)
...
export default function AdminCourseEditPage() {
  ...
  // Load course data (after instructors are loaded)
  useEffect(() => {
    let isMounted = true

    const loadCourse = async () => {
      if (!courseId || isLoadingInstructors) return
      ...
      try {
        ...
        const localCourse = mapApiToLocalCourse(response, instructors)
  ...
  }, [courseId, instructors, isLoadingInstructors])

  // Handle instructor change
  const handleInstructorChange = (instructorId: number) => {
    if (!course) return

    const selectedInstructor = instructors.find(i => i.userId === instructorId)
  ...
  const hasChanges = (): boolean => {
    ...
    return (
      ...
      course.instructorId !== originalCourse.instructorId ||
  ...
  const handleSaveCourse = async () => {
    ...
    try {
      const response = await updateCourseInfo({
        courseId: course.id,
        userId: course.instructorId || userId,
        ...
        // Note: If your API supports instructor change, add it here
        // instructorId: course.instructorId !== originalCourse?.instructorId ? course.instructorId : undefined,
      })
      ...
      <div className="min-h-screen bg-background py-12 px-4 flex items-center justify-center">
        <div className="flex items-center gap-3">
          ...
          <p className="text-muted-foreground">
            {isLoadingInstructors ? 'Loading instructors...' : 'Loading course...'}
          </p>
        </div>
      </div>
    )
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>Manage course details and metadata</CardDescription>
          </CardHeader>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 border border-border rounded-md bg-background text-foreground h-10 mt-1"
                >
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                Manage course structure ({course.modules.length} modules)
              </CardDescription>
            </div>
