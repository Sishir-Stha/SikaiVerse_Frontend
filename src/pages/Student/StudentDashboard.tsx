...
import { getDashboardInfo, type GetDashboardInfoResponse } from '../../api/Student/studentDashboard'
...
useEffect(() => {
  const loadDashboardData = async () => {
    try {
      ...
      // Instead of showing an error, use default zeroed data
      setDashboardData({
        completed: 0,
        inProgress: 0,
        studyTime: 0,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="mb-8 animate-fade-in-up">
            ...
            <p className="text-muted-foreground">
              Welcome back, {user?.username || 'user'}! Continue your learning journey.
            </p>
          </div>
                  ...
                  {isLoading ? (
                    ...
                    <div className="space-y-6">
                      {dashboardData.courseData.map((course, idx) => (
                        <div 
                          key={course.courseId} 
                          className={`pb-6 border-b border-border last:border-0 last:pb-0 animate-stagger-${(idx % 3) + 1} transition-all duration-300 hover:bg-muted/50 rounded-lg p-3`}
                        >
